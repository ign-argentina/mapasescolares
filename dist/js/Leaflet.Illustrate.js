...
L.Illustrate.Textbox = L.RotatableMarker.extend({
	...
	options: {
		...
		textEditable: true,
		textContent: '',
		resetHeight: '', resetWidth: '', resetMarginTop: '', resetMarginLeft: ''
	},
    ...
    _animateZoom: function(t) {
    	// updates the size only on screen, not in printer
    	var zoomOffSet = Math.round((t.zoom - this.addZoom) + 1);
    	if(zoomOffSet == 1){
	    	this._updateSize();    		
    	}
    	else {
    		switch (zoomOffSet) {
	    		case -1:
			    	this._icon.style.height = this.options.resetHeight / 4 + 'px';
			    	this._icon.style.marginTop = this.options.resetMarginTop / 4 + 'px';
			    	this._icon.style.width = this.options.resetWidth / 4 + 'px';
			    	this._icon.style.marginLeft = this.options.resetMarginLeft / 4 + 'px';
	    			break;
	    		case 0:
			    	this._icon.style.height = this.options.resetHeight / 2 + 'px';
			    	this._icon.style.marginTop = this.options.resetMarginTop / 2 + 'px';
			    	this._icon.style.width = this.options.resetWidth / 2 + 'px';
			    	this._icon.style.marginLeft = this.options.resetMarginLeft / 2 + 'px';
	    			break;
	    		case 2:
			    	this._icon.style.height = this.options.resetHeight * 2 + 'px';
			    	this._icon.style.marginTop = this.options.resetMarginTop * 2 + 'px';
	    		...
	    		case 3:
	    			...
	    			break;
	    		case 4:
			    	this._icon.style.height = this.options.resetHeight * 8 + 'px';
			    	this._icon.style.marginTop = this.options.resetMarginTop * 8 + 'px';
			    	this._icon.style.width = this.options.resetWidth * 8  + 'px';
			    	this._icon.style.marginLeft = this.options.resetMarginLeft * 8 + 'px';
	    			break;
	    		default: 
	    			break;		
    		}
	...
	setContent: function(text, top, left) {
		var topOffset = ((typeof top == "undefined") ? Math.round(-(this.getSize().y / 2)) + 'px' : top);
		var leftOffset = ((typeof left == "undefined") ? Math.round(-(this.getSize().x / 2)) + 'px' : left);
		this.getTextarea().innerHTML = text;
		this.options.icon.options.html = '<textarea style="position:relative;left: ' + leftOffset + ';top: ' + topOffset + ';height: ' + this.getSize().y + 'px;width:' + this.getSize().x + 'px">' + text + '</textarea>';
		return this;
	},
	...
	_updateSize: function() {
		...
		if (this._icon) {
			...
			// set original style (size and position)
			this.options.resetHeight = this._icon.style.height.substr(0, this._icon.style.height.length - 2);
			this.options.resetWidth = this._icon.style.width.substr(0, this._icon.style.width.length - 2);
			this.options.resetMarginTop = this._icon.style.marginTop.substr(0, this._icon.style.marginTop.length - 2);
			this.options.resetMarginLeft = this._icon.style.marginLeft.substr(0, this._icon.style.marginLeft.length - 2);
		}
	},
	...
	reSize: function() {
		// updates the size of this.options.icon.options.html with the new size of this._icon for printing 
		this._size.x = this.getTextarea().clientWidth;
		this._size.y = this.getTextarea().clientHeight;
		this.setContent(this.getContent(), this._icon.style.marginTop, this._icon.style.marginLeft);
		this.fire('resize', { size: this._size });
	},
	...
	_enableTyping: function() {
		...
		L.DomEvent.on(textarea, 'mouseup', function() {
			this.reSize();
		}, this);
...
L.Illustrate.Create.Textbox = L.Draw.Rectangle.extend({
	...
	_fireCreatedEvent: function() {
		...
		L.DomUtil.removeClass(styleEditor.options.styleEditorDiv, "editor-enabled"); /* Elimina el panel del StyleEditor cuando terminas de crear el cuadro de texto*/
	},
...
L.Illustrate.Textbox.addInitHook(function() {
	if (L.Illustrate.Edit.Textbox) {
		...
		if (this.options.editable) {
			this.editing.enable();
			console.log('Textbox edit fires');
		}
	}
});
